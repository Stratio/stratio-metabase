...
Feature: Install Discovery
  ...
  @runOnEnv(STRATIO_DISCOVERY_VERSION=0.28.9) ####
  ...
  Scenario: [Basic Installation Discovery][03] Install using config file and cli
    ...
    Then the command output contains 'Installing Marathon app for package [${DISC_PACKAGE:-discovery}] version [${DISC_VERSION:-0.28.9}]'
...
#  @runOnEnv(STRATIO_DISCOVERY_VERSION=0.29.0-SNAPSHOT)
#  Scenario: [Basic Installation Discovery][04] Create config file
#    Given I create file 'config_discovery_0.29.0.json' based on 'schemas/config_discovery_0.28.9.json' as 'json' with:
#      | $.Service.cpus                        | REPLACE | ${DISCOVERY_SERVICE_CPUS:-1}                                             | number  |
#      | $.Service.name                        | UPDATE  | ${DISCOVERY_SERVICE_NAME:-discovery}                                     | n/a     |
#      | $.Service.instances                   | REPLACE | ${DISCOVERY_SERVICE_INSTANCES:-1}                                        | number  |
#      | $.Service.mem                         | REPLACE | ${DISCOVERY_SERVICE_MEM:-2048}                                           | number  |
...
#  Scenario: [Basic Installation Discovery][03] Install using config file and cli
#    Given I open a ssh connection to '${DCOS_CLI_HOST:-dcos-cli.demo.labs.stratio.com}' with user '${CLI_USER:-root}' and password '${CLI_PASSWORD:-stratio}'
#    When I outbound copy 'target/test-classes/config_discovery_${DISC_VERSION:-0.29.0}.json' through a ssh connection to '/tmp'
#    And I run 'dcos package install --yes --package-version=${DISC_VERSION:-0.29.0} --options=/tmp/config_discovery_${DISC_VERSION:-0.29.0}.json ${DISC_PACKAGE:-discovery}' in the ssh connection
#    Then the command output contains 'Installing Marathon app for package [${DISC_PACKAGE:-discovery}] version [${DISC_VERSION:-0.29.0}]'
#    Then the command output contains 'Discovery stack has been installed.'
#    Then I run 'rm -rf /tmp/config_discovery_${DISC_VERSION:-0.29.0}.json' in the ssh connection
#    And I run 'dcos marathon task list discovery | awk '{print $5}' | grep discovery' in the ssh connection and save the value in environment variable 'discoveryTaskId'
#    Then in less than '300' seconds, checking each '10' seconds, the command output 'dcos marathon task show !{discoveryTaskId} | grep TASK_RUNNING | wc -l' contains '1'
